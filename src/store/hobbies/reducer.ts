...
const reducer = (state = INITIAL_STATE, action: HobbiesActionTypes): HobbyState => {
  switch (action.type) {
    ...
    case HobbyTypes.GET_HOBBY_SUCCESS:
      return { ...state, loading: false, selected: action.payload.selected, hobbies: action.payload.hobbies }
    // post hobby
    ...
    case HobbyTypes.DELETE_HOBBY_SUCCESS:
      return { ...state, loading: false, hobbies: state.hobbies.filter(hobby => hobby.id !== action.payload.id) }
    default:
      return state
  }
}
